...
def evaluate(gold, predict, db_dir, etype, kmaps, plug_value, keep_distinct, progress_bar_for_each_datapoint):
    ...
    with open(predict,encoding='utf-8') as f:
        ...
        lines = f.readlines()
        for l in lines:
          #cosql multi-turn evaluation donot need to replace "\n"
            #for i in range(len(lines)):
                #if len(lines[i].strip()) == 0 and i != len(lines) - 1:
                    #lines[i] = "SELECT 1 AS dummy;"
                #print(lines[i])
               # try:
                 # if lines[i].split()[-2].lower() == 'join' or lines[i].split()[-4].lower() == 'join' and i != len(lines) - 1:
                 #     lines[i] = "SELECT 1 AS dummy;"
...
if __name__ == "__main__":
    ...
    # gold = '/data/inspur/base_model_exp/test/Spider/gold_sql.txt'
    # db = '/data/inspur/base_model_exp/test/Spider/test_database'
    #pred = '/data/inspur/base_model_exp/test/result/0309_test/deepseek-33b-spider_test_sft1-spider.txt'
    #log_file = open(f"/data/inspur/LLaMA-cosql-tuning/base_model_exp/case_analysis/log/{args.pred.split('/')[-1].split('.txt')[0]}-log.txt",'w')
    ...
    # df = pd.DataFrame(wrong_sql)
    # df.to_csv("6.7B-instruct-zh-cspider-badcase.csv")